...
BestInPlaceEditor.prototype = {
  ...
  initOptions : function() {
    ...
    self.element.parents().each(function(){
      ...
      self.select2_options   = self.select2_options   || $parent.attr("data-select2-options");
    });
    ...
    self.collection        = self.element.attr("data-collection")          || self.collection || null;
    ...
    self.select2_options   = self.element.attr("data-select2-options")     || self.select2_options || null;
    ...
    if ((self.formType == "select" || self.formType == "checkbox" || self.formType == "select2") && self.collection !== null)
    {
...
/*
    Addon to make best_in_place works with Select2.
    In rails view use like <%= best_in_place @item, :method, :type => :select2, :collection => [{id: "1", text: "Option 1"}, {id: "2", text: "Option 2"}] %>
    You can use any good for select2 "date:" value in :collection.

    Also, you can set all Select2 options as string that will be eval.
    Example: <%= best_in_place @item, :method, :type => :select2, :select2_options => "{width: \"element\", data: [{id: \"1\", text: \"Option 1\"}]}" %>

    Take parts of this this code from alank64/best_in_place
    https://github.com/alank64/best_in_place/commit/4efac699fbbbe5737a6a4da49d30d561ddd5de07
*/
  ...
  "select2" : {
    activateForm : function() {
      ...
      if (this.select2_options !== null){
        eval("s2options = " + this.select2_options);
        this.element.find('input:hidden').select2(s2options).select2("open");
      ...
      this.element.find('input:hidden').on("select2-close", function(e) { 
        that.update(); 
      });
    ...
    getValue :  function() {
      return this.sanitizeValue(this.element.find("input:hidden").val());
    },
